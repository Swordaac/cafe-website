...
router.post('/loyalty/enroll', authSupabase, resolveTenant, auditLog, tenantRateLimiter, ensureTenantExists, async (req, res, next) => {
    try {
...
// Get loyalty status
router.get('/loyalty/status', authSupabase, resolveTenant, auditLog, tenantRateLimiter, ensureTenantExists, async (req, res, next) => {
    try {
        const userId = req.auth?.userId;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/loyalty/redeem', authSupabase, resolveTenantStrict, auditLog, tenantRateLimiter, ensureTenantExists, async (req, res, next) => {
    try {
        const userId = req.auth?.userId;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
